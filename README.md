...
> 網站中的 Aegis CLI 是虛構的資安審查工具，用來示範產品網站的資訊架構與互動設計。指令、旗標與輸出皆為展示內容，不對應真實可安裝的程式。
...
網站上的「MIT Licensed」是展示內容的一部分，描述的是虛構的 Aegis CLI 這個工具，而不是這個 repository。

這份原始碼本身目前沒有宣告授權；未指定授權時預設為保留所有權利。若要開放他人使用，請新增 `LICENSE` 檔案並填入著作權人。
